...
timeoptimize:
	...
	do{	
		...
		for(int i=0; i<workflows.size(); i++) {
			if((int)t == (int)workflows[i]->arrival_time) {
				vp = vertices(workflows[i]->g);
				if(workflows[i]->type == single || workflows[i]->type == pipeline || workflows[i]->type == hybrid)
					workflows[i]->g[*vp.first].status = ready;
				else if(workflows[i]->type == Montage)
					for(int j=0; j<4; j++)
						workflows[i]->g[j].status = ready;
				else if(workflows[i]->type == Ligo)
					for(int j=0; j<9; j++)
						workflows[i]->g[j].status = ready;
				else if(workflows[i]->type == Cybershake) {
